...
try {
    ...
    if (@($notifications | Where-Object type -eq 'Completed').Count -ne 1 -or @($notifications | Where-Object type -eq 'PermissionRequired').Count -ne 1) { throw '通知去重或類型保留失敗。' }
    ...
    if (@($claim).Count -ne 1 -or $claim[0].state -ne 'shown' -or $claim[0].handlerId -ne 'completed-toast') { throw 'Completed 通知 Claim 錯誤。' }
