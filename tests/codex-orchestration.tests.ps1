...
if ($minimal.Status -ne 'PendingUserSetup' -or $minimal.Roles.Planner.Enabled -or $minimal.Roles.Advisor.Enabled -or $minimal.Roles.Designer.Enabled) { throw '精簡模式角色預設錯誤。' }
...
if ((Get-InstallResultSymbol -Status 'PendingUserSetup') -ne '!' -or (Get-InstallResultSymbol -Status 'NotConfigured') -ne '-') { throw 'Workflow 最終摘要符號錯誤。' }
...
foreach ($field in @('pluginManaged', 'pluginPresent', 'pluginUpdatedThisRun', 'workflowManaged', 'workflowConfigured', 'workflowEffective', 'workflowConfigurationSummary')) {
