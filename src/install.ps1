...
if ($Mode -in @('Backup', 'Restore', 'Uninstall')) {
    ...
    $actionScript = Join-Path $ScriptRoot $actionScripts[$Mode]
...
try {
    ...
    try {
        ...
        $ccusage = & (Join-Path $ScriptRoot 'integrations\install-usage-tools.ps1') -SkipPackageInstall:$SkipCcusageInstall -PackageState $ccusageBefore -PassThru
