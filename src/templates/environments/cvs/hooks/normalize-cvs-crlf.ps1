...
while ($null -ne $directory) {
    if (Test-Path -LiteralPath (Join-Path $directory.FullName 'CVS') -PathType Container) { $projectDirectory = $directory }
    $directory = $directory.Parent
}
if ($null -eq $projectDirectory) { exit 0 }
