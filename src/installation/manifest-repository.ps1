function New-InstallationOwnershipManifest {
    ...
    return [ordered]@{
        personal = [ordered]@{
            ...
            context7 = [ordered]@{ Owner = 'CodexSettings'; Category = 'Personal'; Selected = $false; Action = 'LeaveUnchanged'; DiscoveredState = 'Unknown'; LastResult = 'LeftUnchanged'; ManagedPaths = @(); ManagedConfigSections = @('mcp_servers.context7'); ManagedHooks = @(); ManagedExternalState = @('CONTEXT7_API_KEY'); RollbackScope = 'PersonalTransaction' }
