function New-InstallationOwnershipManifest {
    ...
    return [ordered]@{
        ...
        community = [ordered]@{
            windowsUsageNotifications = [ordered]@{ Owner = 'WindowsUsageNotifications'; Category = 'Community'; Selected = [bool]$InstallWindowsNotifications; ManagedPaths = @('hooks/show-codex-notification.ps1'); ManagedConfigSections = @(); ManagedHooks = @('PreToolUse/request_user_input', 'PermissionRequest', 'Stop/Completed'); ManagedExternalState = @('ccusage', 'ccsessions', 'cdaily', 'PowerShellProfiles'); RollbackScope = 'WindowsUsageNotificationTransaction'; scriptPresent = $false; hookConfigured = $false; hookTrusted = $false; hookEffective = $false; directToastShown = $false; lastInvocation = $null; lastResult = 'not-tested' }
            ...
            codexOrchestration = [ordered]@{ Owner = 'CodexOrchestration'; Category = 'Community'; Selected = [bool]$InstallCodexOrchestration; ManagedPaths = @(); ManagedConfigSections = @('workflow'); ManagedHooks = @(); ManagedExternalState = @('marketplace', 'plugin'); RollbackScope = 'CodexOrchestrationTransaction'; pluginStatus = 'SkippedByUser'; workflowRequested = $false; workflowStatus = 'SkippedByUser'; setupPrompt = ''; actionRequired = $false; lastVerified = $null }
            serena = [ordered]@{ Owner = 'Serena'; Category = 'Community'; Selected = [bool]$InstallSerena; ManagedPaths = @('~/.serena/serena_config.yml:web_dashboard_open_on_launch'); ManagedConfigSections = @('mcp_servers.serena'); ManagedHooks = @(); ManagedExternalState = @('uv', 'serena'); RollbackScope = 'SerenaTransaction' }
        }
    }
}
