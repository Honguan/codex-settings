...
try {
    ...
    if (@($installedHooks.hooks.PSObject.Properties.Value | ForEach-Object { @($_) } | Where-Object { (Test-ManagedLineEndingHookEntry $_) -or (Test-ManagedNotificationHookEntry $_) }).Count -ne 0) { throw 'CVS installation must not install Codex Settings hooks.' }
    ...
    if (Test-Path -LiteralPath (Join-Path $globalRoot 'hooks\preserve-line-endings.ps1') -PathType Leaf) { throw 'CVS installation retained the archived line-ending hook script.' }
    if (Test-Path -LiteralPath (Join-Path $globalRoot 'hooks\show-codex-notification.ps1') -PathType Leaf) { throw 'CVS installation retained the archived Windows notification hook script.' }
    ...
    if ($projectCheck.GlobalNotificationStopHookCount -ne 0 -or -not $projectCheck.NotificationCommandConfigured -or $projectCheck.ProjectNotificationStopHookCount -ne 0 -or $projectCheck.EffectiveCompletedNotificationHookCount -ne 1 -or $projectCheck.LegacyCompletedNotificationHookCount -ne 0 -or $projectCheck.StandaloneTokenUsageHookCount -ne 0 -or $projectCheck.ProjectLineEndingPostToolUseHookCount -ne 1 -or $projectCheck.ProjectLineEndingStopHookCount -notin @(0, 1) -or $projectCheck.ProjectLineEndingFinalizeHookCount -ne $projectCheck.ProjectLineEndingStopHookCount -or $projectCheck.LegacyCrlfHookCount -ne 0 -or $projectCheck.LegacyTokenHookCount -ne 0 -or $projectCheck.DuplicateManagedHookCount -ne 0 -or -not $projectCheck.DetachedToastCleanup) { throw 'CVS 專案 Hook 自檢結果不符合 v5.1.5 驗收條件。' }
